...
const Utils = {
	...
	String: {
		...
		customSort: function(str) {
			return Utils.Array.arrayToString(Utils.Array.sortArrayAsc(Utils.String.stringToArray(str)));
		},
		...
		toWordsArray: function(str) {
			let punctuationLess = Utils.String.removeSymbolsRegex(str, /[.,\/#!$%\^&\*;:{}=\-_`~()]/g);
...
function isPalindrome(string) {
	...
	let incomeStr = Utils.String.removeSymbols(string, ' ', ',', '.', '?', '!', '-', ':', ';', "'", '`');
...
const figures = {
	Triangle: function(sideA, sideB, sideC) {
		...
		(this.calculateSquare = function() {
			...
			let cos = {
				sideA: (sideB * sideB + sideC * sideC - sideA * sideA) / (2 * sideB * sideC),
				sideB: (sideA * sideA + sideC * sideC - sideB * sideB) / (2 * sideA * sideC),
				sideC: (sideA * sideA + sideB * sideB - sideC * sideC) / (2 * sideA * sideB)
			};
			...
			const diameter = sideA / sin.angleA || sideB / sin.angleB || sideC / sin.angleC;
...
const numberConverter = {
	...
	fromNegativeDecimalToBinary: function(decimalNum) {
		...
		let resultPositiveBinary = [];
		if (decimalNum < 0) {
			resultPositiveBinary = numberConverter.fromPositiveDecimalToBinary(-(decimalNum + 1));
		} else {
...
const fibonacciNumbers = {
  ...
  calcNumbersRecursion: function (prev, curr, limit) {
    if (limit > 0) {
      return [
        prev,
        ...fibonacciNumbers.calcNumbersRecursion(
          curr,
          prev + curr,
          limit - 1
        ),
